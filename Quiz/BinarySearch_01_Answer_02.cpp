...
int main(void) {
  ...
  int start = 0;
  int end = 1e9; // 10억 // 이렇게 설정하더라도 이진 탐색 수행 과정에서 이 끝점은 자동으로 조율되기 때문에 마찬가지로 최적의 해를 구할 수 있다.

  // 이진 탐색 수행 (반복적)
  int result = 0;
  while (start <= end) {
    long long int total = 0; // 각 떡의 높이는 최대 10억까지의 값으로 구성될 수 있으므로 total 변수는 long long 자료형을 이용해서 초기화할 수 있도록 한다.
    int mid = (start + end) / 2;
