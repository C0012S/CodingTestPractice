...
다익스트라 알고리즘 : 개선된 구현 방법 성능 분석
  ...
  - 노드를 하나씩 꺼내 검사하는 반복문(while 문)은 노드의 개수 V 이상의 횟수로는 처리되지 않습니다.
  ...
  - 직관적으로 전체 과정은 E개의 원소를 우선순위 큐에 넣었다가 모두 빼내는 연산과 매우 유사합니다.
    ...
    - 중복 간선을 포함하지 않는 경우에 이를 O(ElogV)로 정리할 수 있습니다.
      - O(ElogE) → O(Elog(V^2)) → O(2ElogV) → O(ElogV)
        * 그래서 결과적으로 우선순위 큐를 이용했을 때, 이러한 다익스트라 알고리즘의 시간 복잡도는 O(ElogV)라고 표현할 수 있으며, 통상적으로 간선의 개수가 10만 개, 20만 개, 그리고 노드의 개수도 만 개 이상으로 많아진다고 하더라도, 대개 1초 안 쪽의 시간으로 출발 노드부터 다른 모든 노드까지의 최단 거리를 구할 수 있다.
"""
